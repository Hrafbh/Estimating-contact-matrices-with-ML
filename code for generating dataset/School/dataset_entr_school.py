...
for i in range(120):
    ...
    M=M.flatten()
    M=M.tolist()  
    X=X+M    #on ajoute la matrice de contact associée à l'instance i au vecteur X , X est de taille 65 maitenant ,les 64 premiers valeurs represente l'input et la derniere represente l'output
...
df.to_excel('dataset_contact_school.xlsx', sheet_name='dataset',header=[f'feature_{i+1}' for i in range(512)]+[f'mat_{i},{j}'for i in range(1,17) for j in range(1,17)])#run this code to build the dataset