...
#fonction pour génerer les Matrices de contact aléatoirement
##################################################################
def genrate_contact(n):
    C=[]
    for i in range (n):
       C.append(np.random.random((16,16)))
...
#input : pour chaque instance , on calcule la valeur moyenne des vecteurs S,E,I,R pour chaque catégorie d'age
#output : pour chaque instance , on calcule la valeur moyenne de la matrice de contact

C=[]
for i in range(120):
    file_loc=r"E:\Hraf\S8\Projet à Enjeux\Projet Enjeux\matrix_contact_177country\data_contact_school.xls"
    ...
    for i in range(16):
        ...
        S=LA.norm(A[:,4*(i+1)-4])  #Si est la valeur moyenne du vecteur [Si] 
        E=LA.norm(A[:,4*(i+1)-3])
        I=LA.norm(A[:,4*(i+1)-2]) 
        R=LA.norm(A[:,4*(i+1)-1])
        ...
        X.append(I)
        X.append(R)
        # X=[S1,E1,I1,R1,.......,Si,Ei,Ii,Ri,.......Sn,En,In,Rn]  avec Si est la valeur moyenne du vecteur [Si] , X est de taille 64

    M=M*(N.T)


    n=LA.norm(M)   # norme de la matrice de contact
    X.append(n)    #on ajoute la moyenne de la matrice de contact associée à linstance i au vecteur X , X est de taille 65 maitenant ,les 64 premiers valeurs represente l'input et la derniere represente l'output
...
df = pd.DataFrame(C,index=[data[((256-(16*(1+10)))+10)+256*(5*i)][0] for i in range(120)]) 
df.to_excel('dataset_contact_norm_school.xlsx', sheet_name='dataset',header=[f'feature_{i+1}' for i in range(64)]+['norme_mat'])#run this code to build the dataset